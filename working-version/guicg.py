...
'''
leftSideframeCG = mantis.left_sideframeCG()
rightSideframeCG = mantis.right_sideframeCG()
hingePin = mantis.hing_pin()
carbodyWeight = mantis.carbody_weight()
carbodyCG = mantis.carbodyCG()
sideframeWeight = mantis.sideframe_weight()
boomWeight = mantis.boom_weight()
headPin = mantis.head_pin()
upperCG = mantis.upper_CG()
upperWeight = mantis.upper_weight()
counterweightCG = mantis.counterweight_CG()
counterweightExtension = mantis.counterweight_extension()
counterWeight = mantis.counterweight_weight()
headPinWeight = mantis.headpin_weight()
boomCG = mantis.boom_CG()
points_com = np.array([]) #Center of mass (Plot without load and head pin)
points_cog = np.array([]) #Center of Gravity(Plot including load)
weights_com = np.array([])
...
loadX = math.radians(0)
loadY = math.radians(0)
loadOnHook = 10
...
'''
def pointsCalc():
    global leftSideframeCG
    global rightSideframeCG
    global counterweightCG
    global boomCG
    global upperCG
    global weights_cog
    global weights_com
    ...
    #used for boomCG calculation
    roTheta = [[np.cos(np.radians(-boomAngle)),0,np.sin(np.radians(-boomAngle))],
        [0,1,0],
        [-np.sin(np.radians(-boomAngle)),0,np.cos(np.radians(-boomAngle))]
        ]

    #CG calculations for each crane parameter.
    #XY = pitch and roll, roZ = slew Angle
    leftSideframeCG = np.dot(f.trans_y(outriggerExtLeft), leftSideframeCG)
    leftSideframeCG = np.dot(XY, leftSideframeCG)

    rightSideframeCG = np.dot(-f.trans_y(outriggerExtRight), rightSideframeCG)
    rightSideframeCG = np.dot(XY, rightSideframeCG)


    counterweightCG = np.dot(counterweightCG, f.trans_x(0))
    counterweightCG = np.dot(roZ, counterweightCG)
    counterweightCG = np.dot(XY, counterweightCG)

    boomCG[0] = boomCG[0] + (0.4 * boomExtension)
    R_boom = f.matrix_mult([f.trans_x(hingePin[0]), f.trans_y(hingePin[1]),
                               f.trans_z(hingePin[2])])
    boomCG = np.dot(R_boom, boomCG)
    boomCG = np.dot(roZ, boomCG)
    boomCG = np.dot(XY, boomCG)

    upperCG = np.dot(roZ, upperCG)
    upperCG = np.dot(XY, upperCG)

    carbodyCG = np.dot(XY, carbodyCG)

    #Center of Mass arrays
    points_com = np.array([leftSideframeCG, rightSideframeCG, counterweightCG, boomCG, upperCG, carbodyCG])
    weights_com = np.array([sideframeWeight, sideframeWeight,
                        counterWeight, boomWeight, upperWeight, carbodyWeight])

    headPin[1] = headPin[1] + boomExtension
    headPin = np.dot(R_boom, headPin)
    headPin = np.dot(roTheta, headPin)
    headPin = np.dot(roZ, headPin)
    headPin = np.dot(XY, headPin)

    headPinWeight = np.array([loadOnHook*math.cos(loadX), loadOnHook*math.cos(loadY), loadOnHook])
    headPinWeight = np.dot(roZ, headPinWeight)

    #Center of Gravity arrays
    points_cog = np.array([leftSideframeCG, rightSideframeCG, counterweightCG, boomCG, upperCG, carbodyCG, headPin])
    weights_cog = np.array([sideframeWeight, sideframeWeight,
                              counterWeight, boomWeight, upperWeight, carbodyWeight, -headPinWeight[2]])


'''
Calculating x and y points for center of mass 
'''
def c_o_m():
    #global points_com
    px_sum = py_sum = pz_sum = total =  0
    for i in range(0,len(points_com)):
        px_sum = points_com[i][0]*weights_com[i]+px_sum
        py_sum = points_com[i][1]*weights_com[i]+py_sum
        pz_sum = points_com[i][2]*weights_com[i]+pz_sum
        total +=weights_com[i]
    return [px_sum/total,py_sum/total,pz_sum/total,1]

'''
Calculating x and y points for center of gravity 
'''
def c_o_g():
    #global points_com
    px_sum = py_sum = pz_sum = total =  0
    for i in range(0,len(points_cog)):
        px_sum = points_cog[i][0]*weights_cog[i]+px_sum
        py_sum = points_cog[i][1]*weights_cog[i]+py_sum
        pz_sum = points_cog[i][2]*weights_cog[i]+pz_sum
        total +=weights_cog[i]
    return [px_sum/total,py_sum/total,pz_sum/total,1]
